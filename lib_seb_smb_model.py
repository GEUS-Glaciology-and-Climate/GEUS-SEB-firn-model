def [melt_mweq, sublimation_mweq, snowthick] = \
...
def [meltflux, Tsurf, dTsurf, EB_prev, stop] \
...
def  [SRout_mdl, SRnet, T_ice, meltflux_internal, dH_melt_internal] = \
...
def [z_h, z_q, u_star, Re] = RoughSurf(WS, z_0, psi_m1, psi_m2, nu, z_WS, c)
...
def  [z_h, z_q, u_star, Re] = SmoothSurf(WS, z_0, psi_m1, psi_m2, nu, z_WS, c)
...
def [RH, q] = SpecHumSat(RH, T, pres, c)
...
def [QE] = profile_latent_heat_flux(z_r, phi_m, Ri, ro_air, c)
...
def Ri = profile_richardson(z1,z2,T1,T2,q1,q2,u1,u2,p,c)
...
def [QH] = profile_sensible_heat_flux (z_r,phi_m, Ri, ro_air,c)
...
def   [QE,QH, T2m, q_2m, ws_10m, Ri] \
            = profile_SensLatFluxes (z1,z2,t1,t2,q1,q2,u1,u2,p, c)
...
def phi_m = profile_stability(Ri)
