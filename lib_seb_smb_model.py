...
def HHsubsurf(df_aws, c):
    
    time, T, z_T, WS, z_WS, RH, z_RH, pres, SRin, SRout, LRin, LRout, snowfall, \
        rainfall, T_rain, theta, theta_v, q, Tsurf, rho_snow, rho_atm, nu, mu, \
            L, SHF, LHF, Re, theta_2m, q_2m, ws_10m, meltflux, melt_mweq, sublimation_mweq = \
    ...
    rhofirn, snowc, snic, slwc, dgrain, T_ice, grndc, grndd, compaction, \
    ...
    for k in progressbar(range(len(time))):
        ...
        SRnet, T_ice, internal_melting = SRbalance(SRout, SRin, z_icehorizon, snowthick, T_ice, rho, k, c)
        ...
        if snowthick[k] > c.smallno:
            ...
            if snowbkt[k] > c.smallno:
               ...
               z_0 = max(c.z0_old_snow, c.z0_old_snow + (c.z0_ice -c.z0_old_snow)*(rho[0,k] - 600)/(920 - 600))
        ...
        for findbalance in range(1, iter_max_EB):
            ...
            L[k], LHF[k], SHF[k], theta_2m[k], q_2m[k], ws_10m[k], Re[k] \
             = SensLatFluxes_bulk(WS[k], nu[k], q[k], snowthick[k],
        ...
        if (iter_max_EB != 1) & ((findbalance == c.iter_max_EB) & (abs(meltflux[k]) >= 10*c.EB_max)):
        ...
        snowc[:,k], snic[:,k],  slwc[:,k], T_ice[:,k],  zrfrz[:,k], \
        rhofirn[:,k], zsupimp[:, k],  dgrain[:,k],  zrogl[k],  Tsurf[k],  \
        grndc[:,k], grndd[:,k],  pgrndcapc[k], pgrndhflx[k], dH_comp[k],  \
        snowbkt[k], compaction[:,k] = \
            sub.subsurface(Tsurf[k], grndc[:,k-1].copy(), grndd[:, k-1].copy(),
        ...
        # bulk density
        rho[:,k] = (snowc[:, k] + snic[:, k]) / (snowc[:, k]/rhofirn[:, k] + snic[:, k]/c.rho_ice)
    ...
    return L, LHF, SHF, theta_2m, q_2m, ws_10m, Re, \
        melt_mweq, sublimation_mweq, snowc, snic,  slwc, T_ice,  zrfrz, \
        rhofirn, zsupimp,  dgrain,  zrogl,  Tsurf,  \
        grndc, grndd,  pgrndcapc, pgrndhflx, dH_comp,  \
        snowbkt, compaction


def IniRhoSnow(T, WS,  c):
    ...
    if c.rho_snow_scheme == 1:
        rho_snow = 625. + 18.7*(mean_T - c.T_0) + 0.293*(mean_T - c.T_0)**2
    
    # surface snow density by Reeh et al. 2005
    if c.rho_snow_scheme == 2:
        rho_snow = 481. + 4.834*(mean_T-c.T_0)
    ...
    if c.rho_snow_scheme == 4:
        ...
        ind = (T >= 258.16) & (WS >= 5)
        rho_snow[ind] = 50 + 1.7 * (T[ind] - 258.16)**1.5 + 25 + 250 *(1-np.exp(-0.2*(WS[ind] - 5)))
    # surface snow density by Liston et al., 2007
    # only works above -15 degC
    return rho_snow
    

def variables_preparation(df_aws, c):   
    ...
    return time, T, z_T, WS, z_WS, RH, z_RH, pres, SRin, SRout, LRin, LRout, \
        snowfall, rainfall, T_rain, theta, theta_v, q, Tsurf, rho_snow, rho_atm, nu, mu, \
        L, SHF, LHF, Re, theta_2m, q_2m, ws_10m, meltflux, melt_mweq, sublimation_mweq
...
def SurfEnergyBudget (SRnet, LRin, Tsurf, k_eff, thick_first_lay, T_ice, T_rain,
    ...
    # SURFACE ENERGY BUDGET
    meltflux = SRnet[0] - SRnet[1] + LRin - c.em * c.sigma * Tsurf**4 - (1 - c.em) * LRin \
        + SHF + LHF -(k_eff[0]) * (Tsurf- T_ice[1]) / thick_first_lay \
...
def SRbalance (SRout, SRin, z_icehorizon, snowthick, T_ice, rho, k, c):
    ...
    #radiation absorption in snow
    SRnet[:(z_icehorizon)] = (SRin[k] - SRout[k])    *np.exp(-c.ext_snow*np.arange(0, z_icehorizon)*c.dz_ice)

    #radiation absorption in underlying ice
    if z_icehorizon < c.z_ice_max:
        SRnet[z_icehorizon:] = (SRin[k]-SRout[k]) * np.exp(-c.ext_snow*snowthick[k]) \
            * np.exp(-c.ext_ice*(np.arange(z_icehorizon, len(SRnet))*        c.dz_ice - snowthick[k]))
    ...
    T_ice[:c.z_ice_max-1, k] = T_ice[:c.z_ice_max-1, k-1] + c.dt_obs / c.dev / rho[:c.z_ice_max-1,k] / c_i[:c.z_ice_max-1] *        (SRnet[:c.z_ice_max-1] - SRnet[1:c.z_ice_max]) / c.dz_ice
...
def  SmoothSurf(WS, z_0, psi_m1, psi_m2, nu, z_WS, c):
    ...
    z_h = z_0 * np.exp(c.ch1[ind] + c.ch2[ind]*np.log(Re) + c.ch3[ind]*(np.log(Re))**2)  
    z_q = z_0 * np.exp(c.cq1[ind] + c.cq2[ind]*np.log(Re) + c.cq3[ind]*(np.log(Re))**2)
    ...
    if WS > c.WS_lim:
        ...
        if (theta >= Tsurf) & (WS >= c.WS_lim): # stable stratification
            ...
            for i in range(0, c.iter_max_flux):
                psi_m1 = -(c.aa*z_0/L + c.bb*(z_0/L-c.cc/c.dd)*np.exp(-c.dd*z_0/L) + c.bb*c.cc/c.dd)
                psi_m2 = -(c.aa*z_WS/L + c.bb*(z_WS/L-c.cc/c.dd)*np.exp(-c.dd*z_WS/L) + c.bb*c.cc/c.dd)
                psi_h1 = -(c.aa*z_h/L + c.bb*(z_h/L-c.cc/c.dd)*np.exp(-c.dd*z_h/L) + c.bb*c.cc/c.dd)
                psi_h2 = -(c.aa*z_T/L + c.bb*(z_T/L-c.cc/c.dd)*np.exp(-c.dd*z_T/L) + c.bb*c.cc/c.dd)
                psi_q = -(c.aa*z_q/L + c.bb*(z_q/L-c.cc/c.dd)*np.exp(-c.dd*z_q/L) + c.bb*c.cc/c.dd)
                psi_q2 = -(c.aa*z_RH/L + c.bb*(z_RH/L-c.cc/c.dd)*np.exp(-c.dd*z_RH/L) + c.bb*c.cc/c.dd)
                ...
                if (L < c.smallno) | (abs((L_prev - L)) < c.L_dif):
                    ...
                    q_2m = q_surf + q_star /c.kappa * (np.log(2/z_q) - psi_q2 + psi_q)
...
def SpecHumSat(RH, T, pres, c):
    ...
    es_wtr = 10**(-7.90298*(c.T_100 / T-1) + 5.02808 * np.log10(c.T_100 / T) \
    ...
    freezing = (T < c.T_0)  # replacing saturation specific humidity values below melting point
