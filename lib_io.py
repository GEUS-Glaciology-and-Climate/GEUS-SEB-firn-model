...
def load_promice(path_promice):
    ...
    if 'hour' in path_promice:
        try: 
            ...
            df['time'] = pd.to_datetime(df['Year'] * 100000 + df['DayOfYear']*100 + df['HourOfDayUTC'], format='%Y%j%H')
...
def write_2d_netcdf(data, name_var, depth_act, time, c):
    ...
    ds.to_netcdf(c.OutputFolder+'/'+c.RunName+'/'+c.station+'_'+name_var+'.nc')
...
def write_1d_netcdf(data, c, var_list = None, time = None, name_file = 'surface'):
    ...
    for name_var in var_list:
        foo = xr.DataArray(data[name_var].values, coords=[time_days_since], dims=["time"], name=name_var)
        ...
        if name_var == var_list[0]:
            ...
            ds = xr.merge([ds, foo])
    ds.to_netcdf(c.OutputFolder+'/'+c.RunName+'/'+c.station+'_'+name_file+'.nc')
