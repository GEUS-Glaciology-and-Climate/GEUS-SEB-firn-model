...
def plot_var(site, run_name, var_name, ylim=[], zero_surf = True):
    ...
    time = np.datetime64('1900-01-01T00') + np.round((time_org - 1)*24*3600) * np.timedelta64(1, 's')
    ...
    # plotting firn model
    im = ax.pcolormesh(time_grid[:,0::6],
              depth[:,0::6],
              var_array[:,0::6][:,:-1],
...
def track_horizon(time, H_surf, depth_act, compaction, date_start, depth_start, step=1):
    ...
    for i in range(ind_start+step, len(time), step):
        ...
        ind_next = int(interp1d(np.insert(depth_mod,0,0),
                                np.arange(len(depth_mod)+1),
                                kind = 'next')(depth_hor[i]))
        comp_tot = np.sum(comp_mod[ind_next:])
        
        # plus compaction within the layer where the horizon is
        comp = (depth_mod[ind_next] - depth_hor[i])/(depth_mod[ind_next] - depth_mod[ind_next-1]) * comp_mod[ind_next-1]
    ...
    if np.sum(np.isnan(depth_hor))>0:
        depth_hor[np.isnan(depth_hor)] = interp1d(np.argwhere(~np.isnan(depth_hor)).transpose()[0],
                                            depth_hor[~np.isnan(depth_hor)],
                                            kind = 'linear',
                                            fill_value = 'extrapolate')(np.argwhere(np.isnan(depth_hor)).transpose()[0])
...
def evaluate_compaction(site, run_name):
    ...
    df_comp_info = pd.read_csv('Firn viscosity/Compaction_Instrument_Metadata.csv').set_index('sitename')
    
    df_comp_info = df_comp_info.loc[site,['instrument_ID', 'installation_daynumber_YYYYMMDD', 'borehole_top_from_surface_m', 'borehole_bottom_from_surface_m']].reset_index(drop=True).set_index('instrument_ID')
    ...
    for i, ID in enumerate(df_comp_info.index):
        ...
        if ID not in df_comp.index.get_level_values(0).unique():
            ...
            continue
        date_start = pd.to_datetime(str(df_comp_info.loc[ID,'installation_daynumber_YYYYMMDD'])).to_datetime64()
        ...
        df_comp.loc[ID,'borehole_shortening_m'].diff().plot(ax=ax2[i], label='Observation')
        ax2[i].plot(time[:-1], np.diff(depth_2-depth_1)*24, label='Simulated')
        ax2[i].set_ylim(-0.004, 0.001)
        tmp = df_comp.loc[np.isin(df_comp.index.get_level_values(0), 
                                            df_comp_info.index),:].index.get_level_values(1)
    ...
    ax2[i].legend()
    fig2.text(0.03, 0.5, 'Compaction rate (m d$^{-1}$)', ha='center', va='center', rotation='vertical')
...
def plot_summary(df, c, filetag='summary', var_list=None):
    def new_fig():
        ...
        plt.subplots_adjust(left = 0.1, right=0.9, top = 0.97, bottom = 0.1, wspace=0.2, hspace=0.05)
        return fig, ax
    ...
    for i, var in enumerate(var_list):
        ...
        if count == 6:
            ...
            plt.savefig(c.OutputFolder+'/'+c.RunName+'/'+'summary_'+str(count_fig),bbox_inches='tight')
    ...
    if count < 6:
        ...
        for k in range(count+1,len(ax)):
            ax[k].set_axis_off()
        ax[0].set_title(c.station)
        plt.savefig(c.OutputFolder+'/'+c.RunName+'/'+filetag+'_'+str(count_fig),bbox_inches='tight')