...
# elevation-based parametrization
        case 4
            ...
            rho_snow(ind,j) = 50 + 1.7 * (T(ind,j) - 258.16).**1.5 +                25 + 250 *(1-exp(-0.2*(WS(ind) - 5)))
...
def InitializationSubsurface(T_obs, depth_thermistor, T_ice,     time, Tsurf_ini, j, c)
...
try dataArray = textscan(fileID, formatSpec, 'Delimiter', delimiter,    'EmptyValue' ,NaN,'HeaderLines' ,startRow-1, 'ReturnOnError', false)
catch me
    startRow = 2
...
if c.perturb_rho_ini ~= 0
    ...
    perturb_olddensity = olddensity(1:ind_20)        + c.perturb_rho_ini * depth(ind_20)/length(1:ind_20)./ old_thickness(1:ind_20)
    perturb_olddensity_avg = sum(perturb_olddensity.*old_thickness(1:ind_20))        / depth(ind_20)
...
thickweq(1:length(depth)-1) = (depth(2:length(depth))-depth(1:length(depth)-1))     .*olddensity(1:length(depth)-1)/c.rho_water
...
# density on the model depth scale
    for ii = 1:nbins
        ...
        mean_firndensity(ii) = sum(density_mergedscale(ind_in_bin).*thick_mergedscale_m(ind_in_bin))             /sum(thick_mergedscale_m(ind_in_bin))
...
if strcmp(c.station,'NUK_K')
        rhofirn_ini(length(mean_firndensity)+1:length(c.cdel),1) =             860
    ...
    if length(mean_firndensity)<length(c.cdel)
        ...
        rhofirn_ini(length(mean_firndensity)+1:length(c.cdel),1) =             fo(depth_weq(length(mean_firndensity)+1:length(c.cdel)))
        rhofirn_ini(rhofirn_ini>c.rho_ice) = c.rho_ice
...
rho =  (c.cdel *c.rho_water)./    ((snowc_ini *c.rho_water)./rhofirn_ini + (snic_ini *c.rho_water)./c.rho_ice)
...
# EXTRAPOLATING UNTIL DEEP FIRN TEMPERATURE
    if length(newtemp)<length(c.cdel)
        ...
        T_ice(ind,1,j) =             (T1-T2)/(d2-d1)**2*(d2-ind).**2 + T2
...
if c.verbose == 1
    ...
    leg('Observations','Model initial state',        'Prescribed deep \newlinefirn temperature',        'Interpreter','tex',    'Location','NorthOutside')
...
return T_ice, rhofirn_ini,rho,snic_ini, snowc_ini,     slwc_ini, graind_out
...
R = cellfun(@(x) ~isnumeric(x) && ~isnp.logical(x),rawNumericColumns)# Find non-numeric.lls
...
for i = 1:length(varname1)
    for j =1:length(varname2)
        if strcmp(varname1(i),varname2(j))           
            fprintf('#s#s#0.2e\n',char(varname1(i)),                repmat(' ',1,20-length(char(varname1(i)))),                param.(char(varname2(j))))
