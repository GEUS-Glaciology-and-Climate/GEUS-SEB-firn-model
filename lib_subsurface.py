...
def subsurface(pts, pgrndc, pgrndd, pslwc, psnic, psnowc, prhofirn,     ptsoil, pdgrain, zsn, zraind, zsnmel, pTdeep,    snowbkt, c)
...
[psnowc, psnic, pslwc, pdgrain, prhofirn, ptsoil, snowbkt]    = snowfall_new (zsn, psnowc, psnic, pslwc, pdgrain     , prhofirn, ptsoil, pts,snowbkt, zraind, zsnmel,  c)
# Update BV 2018: Density and temperature tracker
        if c.track_density
            [c.rhoCC20_aft_snow(1), c.rhoCC20_aft_snow(2)] =                 Calculate20mAvgDensity(psnowc, psnic, pslwc, snowbkt, prhofirn, ptsoil, c)
        

#BV 2017 updating cdel, cmid and rcdel
c = update_column_properties(c,psnowc, psnic, pslwc)

[psnowc, psnic, pslwc, pdgrain, prhofirn, ptsoil, snowbkt]    = sublimation_new (zsn, psnowc, psnic, pslwc, pdgrain     , prhofirn, ptsoil, snowbkt, c)
# Update BV 2018: Density and temperature tracker
        if c.track_density
...
[psnowc, psnic, pslwc, pdgrain, prhofirn, ptsoil ]     = rainfall_new (zraind, psnowc, psnic, pslwc, pdgrain     , prhofirn, ptsoil, pts, c)
...
[psnowc, psnic, pslwc, snowbkt] =         melting_new (psnowc, psnic, pslwc, zsnmel, snowbkt, ptsoil,prhofirn, c)

# Update BV 2018: Density and temperature tracker
        if c.track_density
             [c.rhoCC20_aft_melt(1), c.rhoCC20_aft_melt(2)]=                  Calculate20mAvgDensity(psnowc, psnic, pslwc, snowbkt, prhofirn, ptsoil, c)
...
[prhofirn, psnowc , psnic, pslwc , pdgrain, zrogl] =    perc_runoff_new (prhofirn, psnowc , psnic, pslwc ,     pdgrain, c)

# Update BV 2018: Density and temp tracker
        if c.track_density
            [c.rhoCC20_aft_runoff(1), c.rhoCC20_aft_runoff(2)] =                 Calculate20mAvgDensity(psnowc, psnic, pslwc, snowbkt, prhofirn, ptsoil, c)
...
[pgrndc, pgrndd, pgrndcapc, pgrndhflx]    = update_tempdiff_params (prhofirn, pTdeep     , psnowc, psnic, ptsoil, zso_cond, zso_capa, c)

return psnowc, psnic, pslwc, ptsoil, zrfrz, prhofirn,    zsupimp, pdgrain, zrogl, psn, pgrndc, pgrndd, pgrndcapc, pgrndhflx,    dH_comp, snowbkt, compaction, c



def densification (pslwc, psnowc , prhofirn, ptsoil, c)
...
#            ! We have neglected surface slope in eqn 6.
    ...
    f1f2(1:c.jpgrnd,1) = 4./( 1 + 60.*(pslwc./ (psnowc + c.smallno))         .* (prhofirn ./ c.rho_water) )# ! f2 = 4
    
    eta_firn(1:c.jpgrnd,1)   = f1f2 .* 7.62237e+6 .* (c.a_dens * prhofirn / 250 )         .* exp( 0.1.*(c.T_0 - ptsoil) + c.b_dens * 0.023.*prhofirn )
...
#     prhofirn = prhofirn + c.zdtime.*#         MO .* C .* c.accum_AWS * c.rho_water/365/24/3600 .* c.g .* (c.rho_ice - prhofirn)#         .* exp(-Ec./c.R./ptsoil + Eg./c.R./ptsoil)
...
def melting_new (psnowc, psnic, pslwc, zsnmel, snowbkt, ptsoil, prhofirn, c)
...
return psnowc, psnic, pslwc, snowbkt
...
# than shallow layers
    ...
    if sum(isshallow+isdeep) < length(mid_point_weq)
       isbetween = find(and(~isshallow,~isdeep))
        crit_6(isbetween) = interp1(            [c.deep_firn_lim, c.shallow_firn_lim],            [1, 0], mid_point_weq(isbetween))
...
# final rating:
    ...
    w6 = 2
    w7 = 3
    crit = (w1.*crit_1 + w2.*crit_2+ w3.*crit_3 +        w4.*crit_4 + w5.*crit_5 + w6 .* crit_6+ w7 .* crit_7)./(w1+w2+w3+w4+w5+w6+w7)
...
# layer of index i_merg and i_merg+1 are merged
    if (psnowc(i_merg+1) + psnowc(i_merg))> c.smallno
        ...
        pdgrain(i_merg+1) = (psnowc(i_merg) * pdgrain(i_merg) + psnowc(i_merg+1) * pdgrain(i_merg+1))            /(psnowc(i_merg+1) + psnowc(i_merg))
        prhofirn(i_merg+1) = (psnowc(i_merg+1) + psnowc(i_merg))            / (psnowc(i_merg)/prhofirn(i_merg) +  psnowc(i_merg+1)/prhofirn(i_merg+1))
    else
        pdgrain(i_merg+1) = -999
        prhofirn(i_merg+1) = -999
    
    ptsoil(i_merg+1) = ((psnic(i_merg) + psnowc(i_merg) + pslwc(i_merg)) * ptsoil(i_merg)         + (psnic(i_merg+1) + psnowc(i_merg+1) + pslwc(i_merg+1)) * ptsoil(i_merg+1))         /(psnic(i_merg) + psnowc(i_merg) + pslwc(i_merg) + psnic(i_merg+1) + psnowc(i_merg+1) + pslwc(i_merg+1))
...
# then we merge this layer with the layer below
            if (psnowc(jk+1) + psnowc(jk))~=0
                pdgrain(jk+1) = (psnowc(jk) * pdgrain(jk) + psnowc(jk+1) * pdgrain(jk+1))                    /(psnowc(jk+1) + psnowc(jk))
                prhofirn(jk+1) = (psnowc(jk+1) + psnowc(jk))                    / (psnowc(jk)/prhofirn(jk) +  psnowc(jk+1)/prhofirn(jk+1))
            else
                pdgrain(jk+1) = c.dgrainNew
                prhofirn(jk+1) = c.rho_ice
            
            ptsoil(jk+1) = ((psnic(jk) + psnowc(jk) + pslwc(jk)) * ptsoil(jk)                 + (psnic(jk+1) + psnowc(jk+1) + pslwc(jk+1)) * ptsoil(jk+1))                 /(psnic(jk) + psnowc(jk) + pslwc(jk) + psnic(jk+1) + psnowc(jk+1) + pslwc(jk+1))
...
# elsewhere
        ...
        [psnic, psnowc, pslwc, pdgrain, prhofirn, ptsoil] =             split_layer(psnic, psnowc, pslwc, pdgrain, prhofirn, ptsoil, c)
...
if zraind > c.smallno
    ...
    ptsoil(1) = ((psnowc(1) + psnic(1) + pslwc(1)) * ptsoil(1)  + zraind * T_rain_in)         / (psnowc(1) + psnic(1) + pslwc(1) + zraind)
...
zpotref(1:c.jpgrnd) = c.Ck *     coldcontent .* cpiceF(ptsoil) .* cfrozen / c.L_fus
...
ptsoil(1:c.jpgrnd) = ptsoil(1:c.jpgrnd) +     zrfrz(1:c.jpgrnd).* c.L_fus./( cpiceF(ptsoil).*(psnic(1:c.jpgrnd)-zrfrz(1:c.jpgrnd) +    psnowc(1:c.jpgrnd)) )
# pts = ptsoil(1)

return psnic, pslwc, ptsoil, zrfrz

def snowfall_new (zsn, psnowc, psnic, pslwc, pdgrain     , prhofirn, ptsoil, pts, snowbkt, zraind, zsnmel, c)
...
#             ptsoil(2:) = ptsoil(1:-1)
#         else
            [psnic, psnowc, pslwc, pdgrain, prhofirn, ptsoil] =                 merge_layer(psnic, psnowc, pslwc, pdgrain, prhofirn, ptsoil,c)
#         
#         disp('SNOWFALL')
#         disp(snowbkt)
        psnowc(1) = snowbkt
...
# mass-weighted average for temperature
    ptsoil(1) = (ptsoil(1)*(psnowc(1)+pslwc(1)+psnic(1)) + min(pts,c.T_0)*snowbkt)        /(psnowc(1)+pslwc(1)+psnic(1)+ snowbkt)

#snow-mass-weighted average for grain size
    pdgrain(1) = (psnowc(1) * pdgrain(1) + snowbkt * c.dgrainNew)            /(psnowc(1) + snowbkt)
    
# volume-weighted average for density
    prhofirn(1) = (psnowc(1) + snowbkt)        / (snowbkt/c.rho_fresh_snow +  psnowc(1)/prhofirn(1))
 
   psnowc(1) = psnowc(1) + snowbkt
   snowbkt = 0       
return psnowc, psnic, pslwc, pdgrain, prhofirn, ptsoil, snowbkt    
...
#=========================================================================   
    ...
    if and(sum(thickness_weq > c.lim_new_lay*2) > 0,            sum(thickness_weq) > c.min_tot_thick)
...
def sublimation_new (zsn, psnowc, psnic, pslwc, pdgrain     , prhofirn, ptsoil, snowbkt, c)
...
#then merging layer 1 and 2 and placing it in layer 2
                pdgrain(2) = (psnowc(1) * pdgrain(1) + psnowc(2) * pdgrain(2))                    /(psnowc(2) + psnowc(1))
                prhofirn(2) = (psnowc(2) + psnowc(1))                    / (psnowc(1)/prhofirn(1) +  psnowc(2)/prhofirn(2))
#                Update PLA
                ptsoil(2) = ((psnic(1) + psnowc(1) + pslwc(1)) * ptsoil(1)                     + (psnic(2) + psnowc(2) + pslwc(2)) * ptsoil(2))                     /(psnic(1) + psnowc(1) + pslwc(1) + psnic(2) + psnowc(2) + pslwc(2))
...
rho_next(1:c.jpgrnd-1) = (psnic(2:c.jpgrnd)+psnowc(2:c.jpgrnd))./    ( psnic(2:c.jpgrnd)/c.rho_ice + psnowc(2:c.jpgrnd) ./ prhofirn(2:c.jpgrnd) )
...
ptsoil(isfrozen==1) = ptsoil(isfrozen==1) +     SIform * c.L_fus ./ (c.cdel(isfrozen==1).*cpiceF(ptsoil(isfrozen==1)))

return ptsoil , psnic, pslwc, zsupimp

def tsoil_diffusion (pts, pgrndc, pgrndd, ptsoil, c)
...
def update_tempdiff_params (prhofirn, pTdeep                        , psnowc, psnic, ptsoil, zso_cond, zso_capa, c)
...
snowV1(2:c.jpgrnd) = 0.5*psnowc(2:c.jpgrnd)  .*    c.rho_water./prhofirn(2:c.jpgrnd)
# volume of snow in lower half layer:
snowV2(2:c.jpgrnd - 1) = 0.5*psnowc(3:c.jpgrnd)*c.rho_water./prhofirn(3:c.jpgrnd)
# volume of ice in both half layers:
iceV(2:c.jpgrnd - 1)  = 0.5*(psnic(2:c.jpgrnd - 1) + psnic(3:c.jpgrnd ))    .*c.rh2oice
...
zcapa = snow_frac_lay_1 .* zsn_capaF(prhofirn, ptsoil) +    snow_frac_lay_2 .*     zsn_capaF(prhofirn([2:c.jpgrnd, c.jpgrnd]), ptsoil([2:c.jpgrnd, c.jpgrnd]))     + ice_frac .* zso_capa
...
zkappa = 1./     ( snow_frac_lay_1./zsn_condF(prhofirn, c) +     snow_frac_lay_2./zsn_condF(prhofirn([2:c.jpgrnd, c.jpgrnd]), c) +     ice_frac./zso_cond )
...
c.cdelV(1:c.jpgrnd) = psnowc(1:c.jpgrnd) *c .rho_water./prhofirn(1:c.jpgrnd)     + psnic(1:c.jpgrnd)*c.rh2oice
# !!! Why here not totalV ?
...
zcapa_abs_sublayer = zsn_capaF(c.rho_ice,pTdeep) * c.cdel(c.jpgrnd)     * c.rho_water / c.rho_ice / c.zdtime
...
pgrndhflx=zkappa_abs(1) * (pgrndc(1)              +(pgrndd(1)-1) * ptsoil(1))
pgrndcapc=(zcapa_abs(1) * c.zdtime+                    c.zdtime * (1-pgrndd(1)) * zkappa_abs(1))
...
def calc_snowdepth1D (psnowc, psnic, snowbkt, c)
